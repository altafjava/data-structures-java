...
public class FrequencyValue {
	public static void main(String[] args) {
		int[] nums = { 1, 2, 3, 4 };
//		int[] nums = { 1, 1, 2, 3 };
		FrequencyValue frequencyValue = new FrequencyValue();
		int[] frequencyValues = frequencyValue.decompressRLElist(nums);
		for (int fv : frequencyValues) {
			System.out.print(fv + " ");
		}
	}
	...
	public int[] decompressRLElist(int[] nums) {
		...
		int[] arr = new int[totalFrequency];
		int k = 0;
		for (int i = 0; i < nums.length; i += 2) {
			int freq = nums[i];
			int val = nums[i + 1];
